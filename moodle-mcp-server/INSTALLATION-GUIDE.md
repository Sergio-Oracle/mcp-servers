...
Le serveur MCP Moodle peut fonctionner en **mode local** (stdio) ou en **mode réseau** (TCP via socat/ncat) pour permettre l'accès depuis plusieurs machines.
...
Le serveur MCP Moodle peut être déployé en réseau pour permettre à plusieurs utilisateurs d'y accéder simultanément :
...
Le serveur utilise `socat` pour exposer le serveur MCP sur le réseau :

```bash
# Installation de socat
sudo apt install socat

# Le script start-moodle-mcp.sh démarre automatiquement le serveur
cd /home/sergio/mcp-servers/moodle-mcp-server
./start-moodle-mcp.sh
```

Le serveur écoute sur le port **3000** et accepte les connexions réseau.

### Configuration des Clients

#### Windows

**1. Installation de nmap (inclut ncat)**
```powershell
# Télécharger depuis: https://nmap.org/download.html
# Installer Nmap avec toutes les options

# Ajouter au PATH système :
# Panneau de configuration > Système > Paramètres système avancés
# > Variables d'environnement > Path > Ajouter :
C:\Program Files (x86)\Nmap
```

**2. Configuration Claude Desktop**

Emplacement du fichier : `%APPDATA%\Claude\claude_desktop_config.json`
...
# Vérifier que ncat est dans le PATH
where ncat
```

#### Ubuntu/Linux

**1. Installation de nmap (inclut nc/ncat)**
```bash
# Installation
sudo apt update
...
Emplacement du fichier : `~/.config/Claude/claude_desktop_config.json`
...
**Note** : Vous pouvez aussi utiliser `ncat` au lieu de `nc` sur Ubuntu.
...
Emplacement du fichier : `~/Library/Application Support/Claude/claude_desktop_config.json`
...
#### Problème : "ncat: command not found" (Windows)

**Solutions :**
```powershell
# 1. Vérifier l'installation de Nmap
dir "C:\Program Files (x86)\Nmap\ncat.exe"

# 2. Ajouter au PATH manuellement
$env:Path += ";C:\Program Files (x86)\Nmap"

# 3. Redémarrer Claude Desktop après modification du PATH
```

#### Problème : Le serveur se ferme après déconnexion

**Solution :**
```bash
# Utiliser screen ou tmux pour maintenir le serveur actif
screen -S moodle-mcp
./start-moodle-mcp.sh
...
⚠️ **Important** : Le serveur MCP expose l'accès à votre instance Moodle. Prenez les précautions suivantes :

1. **Firewall** : Limitez l'accès au port 3000 aux IPs de confiance
   ```bash
   sudo ufw allow from 192.168.1.0/24 to any port 3000
   ```

2. **Réseau local uniquement** : N'exposez pas le port sur Internet
   
3. **Token Moodle** : Utilisez un token avec des permissions limitées

4. **SSL/TLS** : Pour un déploiement en production, ajoutez une couche SSL :
   ```bash
   # Exemple avec stunnel
...
5. **Mode réseau** : En mode réseau, plusieurs utilisateurs peuvent accéder au même serveur MCP simultanément via TCP.
...
- **Script de démarrage réseau** : `start-moodle-mcp.sh`
...
**Auteur** : Serge (RTN)  
**Mode** : Local (stdio) et Réseau (TCP)
