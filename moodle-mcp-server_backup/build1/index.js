...
class MoodleMcpServer {
    ...
    async getSubmissions(args) {
        ...
        const submissionsPromises = targetAssignments.map(async (assignment) => {
            ...
            const processedSubmissions = targetSubmissions.map((submission) => {
                const studentGrade = grades.find((g) => g.userid === submission.userid);
                return {
    ...
    async getSubmissionContent(args) {
        ...
        try {
            ...
            for (const plugin of plugins) {
                ...
                if (plugin.type === 'onlinetext') {
                    const textField = plugin.editorfields?.find((field) => field.name === 'onlinetext');
                    if (textField) {
                ...
                if (plugin.type === 'file') {
                    const filesList = plugin.fileareas?.find((area) => area.area === 'submission_files');
